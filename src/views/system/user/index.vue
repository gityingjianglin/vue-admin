  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="用户账号" key="userName" prop="userName" v-if="columns[1].visible" :show-overflow-tooltip="true" />
          <el-table-column label="用户名称"  key="nickName" prop="nickName" v-if="columns[2].visible" :show-overflow-tooltip="true" />
          <el-table-column label="所属应用"  key="clientId" prop="userContent" v-if="columns[3].visible" width="180" >
             <template slot-scope="scope">
                 <el-popover placement="top-start" title="" style="max-width:300px"
                 trigger="hover"
                 popper-class="poperClass">
                  <div>{{scope.row.userContent}}</div>
                  <span slot="reference" v-if="scope.row.userContent">
                    <!-- <span >{{ scope.row.userContent.substr(0,13)+'...'}} </span> -->
                    <span v-if="scope.row.userContent.length > 13">{{ scope.row.userContent.substr(0,13)+'...'}} </span>
                    <span v-else>{{ scope.row.userContent}} </span>
                  </span>
                </el-popover>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="手机号码" :prop="form.userType === '01'? 'phoneNumber' :''">
              <el-input v-model="form.phoneNumber" placeholder="请输入手机号码" maxlength="11" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="邮箱" :prop="form.userType === '01'? 'email' :''">
...
<script>
import { listUser, getUser, delUser, addUser, updateUser, resetUserPwd, changeUserStatus, getAppInfo, getUserClient, cancleAuthorize } from "@/api/system/user";
      ...
      listUser(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
          ...
          this.userList.forEach (el => {
            ...
            if(el.userAccounts.length > 0) {
              el.userAccounts.forEach(item => {
                item.content = item.appName + item.clientName
                el.userContent += item.content + '\xa0\xa0\xa0\xa0'
              })
