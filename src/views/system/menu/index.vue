<template>
  <div class="app-container">
    ...
    >
      <el-table-column prop="menuName" label="菜单名称" :show-overflow-tooltip="true" width="140"></el-table-column>
      ...
      <el-table-column label="应用名称"  prop="appName" :show-overflow-tooltip="true" >
        <!-- <template slot-scope="scope">
          <span v-if="scope.row.appName">{{scope.row.appName}}的{{scope.row.clientName}}</span>
        </template> -->
      </el-table-column>
      <el-table-column label="客户端名称"  prop="clientName" :show-overflow-tooltip="true" >
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="680px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="应用名称" prop="appId">
              <span v-if="form.menuId ">{{form.appName}}</span>
              <el-select v-model="form.appId"   placeholder="请选择"
              filterable
              @change="addAppChange" v-else>
                <el-option
                v-for="item in AppList"
                :key="item.appId"
                :label="item.appName"
          ...
          <el-col :span="24">
            <el-form-item label="客户端名称" prop="clientId">
              <span v-if="form.menuId ">{{form.clientName}}</span>
              <el-select v-model="form.clientId" @change="addClientChange"
              filterable
...
<script>
import { listMenu, getMenu, delMenu, addMenu, updateMenu, getAppInfo } from "@/api/system/menu";
...
export default {
  ...
  methods: {
    ...
    /** 查询菜单下拉树结构 */
    getTreeselect(clientId) {
      listMenu({clientId:clientId}).then(response => {
        this.menuOptions = [];
        const menu = { menuId: 0, menuName: '主类目', children: [] };
        console.log(response.data, 'menuId');
    ...
    handleUpdate(row) {
      this.reset();
      this.getTreeselect(row.clientId);
